...
\begin{document}
    ...
    \begin{example}[NFA]
        ...
        \begin{figure}[H]
            ...
            \caption{L'immagine rappresenta l'esecuzione dell'NFA descritto.}
        \end{figure}
    ...
    \begin{frameddefn}{Stringhe accettate (GNFA)}
        ...
        \begin{itemize}
            ...
            \item $\forall i \in [1, n - 1] \quad w_i \in L(\delta(q_i, q_{i + 1}))$, ovvero, $w_i$ deve far parte del linguaggio rappresentato dall'espressione regolare sull'arco da $q_i$ a $q_{i + 1}$
    ...
    \proofind{
        ...
        $k = 2$; se $G$ ha solo 2 stati, allora necessariamente $Q = \{q_\mathrm{start}, q_\mathrm{accept}\}$, e dunque $\ttt{convertGNFAtoRegEx}(G) = \delta(q_\mathrm{start}, q_\mathrm{accept})$, alla riga 3.
    }{
    ...
    \begin{framedprinc}[label={pigeonhole}]{Principio della piccionaia}
        ...
        In altri termini, avendo una piccionaia con $m$ caselle, non è possibile inserire più di $m$ piccioni al suo interno: due volatili dovranno necessariamente condividere la casella.
    \end{framedprinc}

    \begin{framedlem}{Pumping lemma}
        Sia $A$ un linguaggio regolare; allora, esiste un $p \in \N$, detta \tbf{lunghezza del pumping}, tale che per ogni stringa $s \in A$ tale per cui $|s| \ge p$, esistono 3 stringhe $x, y, z \in A \mid s = xyz$, soddisfacenti le seguenti condizioni:
    ...
    \begin{frameddefn}{Grammatiche context-free}
        ...
        Le CFG sono della forma $$\left . \begin{array}{c} X \to Y \\ \vdots \end{array} \right .$$ dove $X, Y \in V$, e $X \to Y$ è una regola della CFG.

        Una grammatica è un insieme di regole di sostituzione di stringhe, in grado di produrre stringhe a partire da una variabile iniziale, mediante la sequenza di sostituzioni che definisce la CFG. In particolare, partendo dalla variabile iniziale, vengono sostituiti gli elementi alla sinistra del simbolo $\to$, con quelli alla destra.

        Due regole $X \to Y, X \to Z \in R$ possono essere accorpate con $X \to Y | Z$. Siano $u, v, w$ stringhe di variabili in $V$, e $A \to w \in R$; si dice che $uAv$ \tbf{produce} $uwv$, denotato con $uAv \implies uwv$. Se $u = v$, oppure esistono stringhe $u_1, \ldots, u_k$ con $k \ge 0$ tali che $$u \implies u_1 \implies \ldots \implies u_k \implies v$$ si dice che $u$ \tbf{deriva} $v$, denotato con $u \ximplies{x} v$.
    \end{frameddefn}
    ...
    \begin{frameddefn}{Linguaggio di una grammatica}
        Data una grammatica $G$, il \tbf{linguaggio di $G$} è l'insieme delle stringhe che la grammatica $G$ è in grado di generare, ed è denotato con $L(G)$. In simboli, data una grammatica $G$, si ha che $$L(G) = \{w \in \Sigma ^* \mid S \ximplies{*} w\}$$
    ...
    \begin{frameddefn}{Ambiguità}
        Sia $G$ una grammatica; se esistono due stringhe $u, v$ con $u \neq v$, tali che esiste una terza stringa $z$ derivata a sinistra, si dice che $G$ genera $z$ \tbf{ambiguamente}, e $G$ si dice essere \tbf{ambigua} se genera strighe ambiguamente.
    \end{frameddefn}

    \begin{frameddefn}{Linguaggi inerentemente ambigui}
        Un linguaggio si dice essere \tbf{inerentemente ambiguo} se non esistono grammatiche non ambigue che lo possa generare
    \end{frameddefn}
    ...
    \begin{frameddefn}{Forma normale di Chomsky}
        ...
        % \begin{enumerate}
            ...
            \item avere regole della forma $X \to S \in R$ per qualche $X \in V$, ovvero avere $S$ alla destra di una regola.
        \end{enumerate}
    \end{frameddefn}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item aggiungere $S_0 \in V$, e $S_0 \to S \in R$, in modo da non avere$S$ alla destra di nessuna regola in $R$;
            \item data una variabile $A \in V - \{S\}$ tale che esiste una regola $A \to \varepsilon \in R$, tale regola viene rimossa da $R$, e per ogni regola della forma $X \to uAv \in R$ per qualche $X \in V$ e $u, v$ stringhe di variabili e terminali, viene aggiunta $X \to uv \in R$; se $X \to A \in R$, allora viene aggiunta $X \to \varepsilon \in R$, solo se quest'ultima non era stata precedentemente rimossa
            \item per ogni regola unitaria $A \to B \in R$, quest'ultima viene rimossa, ed ogni regola $B \to u \in R$ per qualche stringa $u$, viene aggiunta $A \to u \in R$, solo se questa non era una regola unitaria precedentemente rimossa;
            \item infine, ogni regola restante $A \to u_1 \ldots u_k$ con $k \ge 3$ (e $u_1, \ldots, u_k$ variabili o terminali) viene rimpiazzata con $$A \to u_1 A_1 , \ldots A_1 \to u_2 A_2 \to A_2 \to u_3 A_3, \ldots, A_{k - 2} \to u_{k - 1}u_k$$, e $A_1, \ldots, A_{k - 2} \in V$; ogni terminale TODO
        \end{itemize}
