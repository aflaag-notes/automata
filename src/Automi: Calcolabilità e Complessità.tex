...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item $M$ esegue $\ttt{findPathGraph} \rbk{G_{N, w}, C_0, C_\mathrm{accept}, \ceil{2^{O(f(n))}}}$;
            \item $M$ accetta se e solo se l'istruzione precedente restituisce \ttt{True}.
        \end{itemize}

        Si noti che, poiché $\ttt{findPathGraph}(G, x, y, k)$ restituisce \ttt{True} se e solo se in $G$ è presente un cammino $x \to y$ lungo al massimo $2^k$, e poiché ogni cammino in un grafo ha lunghezza $O(\abs{V})$, $M$ pone $k = \ceil{2^{O(f(n))}}$ poiché TODO DA FINIRE
        Allora, per costruzione di $M$, si ha che $$w \in L(M) \iff \ttt{findPathGraph} \rbk{G_{N, w}, C_0, C_\mathrm{accept}, \ceil{2^{O(f(n))}}} = \ttt{True} \iff w \in L(N)$$ per costruzione di $G_{N, w}$, dunque $N$ ed $M$ sono equivalenti. Inoltre, per ragionamento analogo alla \cref{path in log sq}, si ha che il costo di spazio di $M$ è pari a TODO DA FINIRE
    \end{proof}
