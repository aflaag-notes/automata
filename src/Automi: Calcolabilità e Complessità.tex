...
\begin{document}
    ...
    \begin{framedobs}{Gerarchia delle classi di tempo}
        TODO METTI QUESTA OBS NEL POSTO GIUSTO (controlla teoremi di gerarchia) E NOMINA I TEOREMI
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \draw (0,0.9) circle [x radius=2, y radius=2] (0,2.2) node {$\Pclass = \coPclass$};
                \draw (-0.7,1.15) circle [x radius=3, y radius=2.3, rotate=-20] (-2.55,2.1) node {\NPclass};
                \draw (0.7,1.15) circle [x radius=3, y radius=2.3, rotate=20] (2.55,2.1) node {\coNPclass};
                ...
                \draw (0,0) circle [x radius=0, y radius=0] (0,7.45) node {\DEC};
                \draw (0.7,3.1) circle [x radius=6, y radius=5, rotate=20] (5.15,5.5) node {\coREC};
                \draw (-0.7,3.1) circle [x radius=6, y radius=5, rotate=-20] (-5.15,5.5) node {\REC};
                \draw (0,3.8) circle [x radius=7, y radius=6] (0,9) node {$\powerset(\Sigma^*)$};
           \end{tikzpicture}
        \end{center}
    \end{framedobs}
    ...
    \begin{proof}
        % Per il \cref{path in np-complete}, il \cref{path in p}, ed il \cref{p w red}, segue che $$\soe{l}{\forall L \in \NLSPACE \quad L \leq_\mathrm L PATH \\ PATH \in \Pclass} \implies \forall L \in \NLSPACE \quad L \in \Pclass \iff \NLSPACE \subseteq \Pclass$$
        % Per il \cref{path in nl-complete}, dato un linguaggio $L \in \NLSPACE$, si ha che $L \leq_\mathrm L PATH$, e dunque per definizione esiste un trasduttore di spazio logaritmico $T$ tale da realizzare la riduzione descritta; allora, poiché $\LSPACE \in \Pclass$ per il \cref{l in p}, si ha che $T$ è anche tale da essere una macchina di Turing che computa in tempo polinomiale, e poiché realizza la riduzione di $L$ a $PATH$, si verifica che $L \leq_\mathrm P PATH$
    \end{proof}
