...
\begin{document}
    ...
    \begin{frameddefn}[label={space complexity}]{Complessità di spazio di una \TM}
        Sia $D$ un decisore; si definisce \tbf{complessità di spazio} di $D$ la funzione $\func{f}{\N}{\N}$ TODO DA FINIRE
    \end{frameddefn}
    ...
    \begin{frameddefn}{Classe \DSPACE}
        Data una funzione $\func{s}{\N}{\R^+}$, si definisce \tbf{classe di complessità di spazio} $\DSPACE(s(n))$ l'insieme dei linguaggi decidibili da una macchina di Turing come descritta nell'\cref{space model} in $O(s(n))$.
    \end{frameddefn}

    \subsection{Classe \LSPACE}

    \begin{frameddefn}{Classe \LSPACE}
        Si definisce \tbf{classe dei linguaggi decidibili in tempo logaritmico} da una \TM come descritta nell'\cref{space model} il seguente insieme $$\LSPACE := \displaystyle \bigcup_{k \in \N}{\DSPACE(\log(n))}$$
    \end{frameddefn}
