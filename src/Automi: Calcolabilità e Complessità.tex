...
\begin{document}
    ...
    \begin{framedthm}{Teorema di gerarchia di spazio}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item siano $\Gamma_D$ e $\Gamma_M$ rispettivamente gli alfabeti di nastro di $D$ ed $M$; poiché $D$ ha un alfabeto di nastro fissato, TODO DA FINIRE
        \end{itemize}

        Questo dimostra che $D$ è un decisore di spazio $O(f(n))$, e sia $L := L(D)$.

        Per giustificare la scelta della codifica di $w = \abk{M} \ttt 1 \ttt 0 ^n$ in input a $D$, si ipotizzi che $M$ computi in spazio $o(f(n))$ (dunque utilizzi meno di $f(n)$ celle); per definizione di $o(f(n))$, l'uso di spazio di $M$ potrebbe essere superiore ad $f(n)$ per valori piccoli di $n$, quando il comportamento asintotico non è ancora stato raggiunto. Di conseguenza, potrebbe verificarsi che $D$ non abbia abbastanza spazio per eseguire $M$ fino al completamento, dunque rifiutando prematuramente, rischiando inoltre di decidere lo stesso lingauggio che decide $M$, condizione per cui il teorema non risulterebbe più verificato. Allora, per garantire che $D$ decida il complemento del linguaggio riconosciuto da $M$, viene inserita una stringa $\ttt 1 \ttt 0^*$ al termine della codifica $\abk{M}$ nell'input di $D$, di modo che quando $M$ computa realmente in $o(f(n))$, $D$ TODO DA FINIRE NON HO CAPITO CHE C'ENTRA STA COSA 

        Per assurdo, sia $M_L$ una macchina tale da decidere $L$ in spazio $g(n)$, con $g(n) = o(f(n))$, e dunque per definizione esistono $c, n_0 \in \N - \{0 \}$ tali per cui $\forall n \ge n_0 \quad g(n) < c \cdot f(n)$. Si assuma inoltre che la codifica di $M_L$ è tale che 

    \end{proof}

    \begin{framedcor}{TODO INVENTA NOME}
