...
\begin{document}
    ...
    \begin{frameddefn}[breakable]{\CFG}
        ...
        dove $X \in V$ e $X \to Y \in R$ è una regola della \CFG. Due regole $X \to Y, X \to Z \in R$ possono essere accorpate con il simbolismo $X \to Y \smid Z$.
        ...
        \begin{center}
            \begin{tabular}{crcl}
                $G:$ & $S$ & $\to$ & $A \smid B \smid AB \smid BA$ \\
                     & $A$ & $\to$ & $\ttt 0 \smid \ttt 0 A \ttt 0 \smid \ttt 0 \ttt A \ttt 1 \smid \ttt 1 A \ttt 0 \smid \ttt 1 A \ttt 1$ \\
                     & $B$ & $\to$ & $\ttt 1 \smid \ttt 0 B \ttt 0 \smid \ttt 0 B \ttt 1 \smid \ttt 1 B \ttt 0 \smid \ttt 1 B \ttt 1$
            \end{tabular}
        \end{center}
        
        e si noti che le stringhe derivate da $A$ sono tutte le stringhe aventi lunghezza dispari ed uno \ttt 0 al centro, mentre le stringhe derivate da $B$ sono tutte le stringhe aventi lunghezza dispari ed un \ttt 1 al centro.
    ...
    \begin{proof}
        ...
        Allora $M$, per costruzione, è in grado di decidere $HAMPATH$. Si noti che i lnumero di cammini possibili in un grafo è $O(2^{n^k})$ per qualche $k \in \N$, e poiché controllare che un cammino sia hamiltoniano richiede tempo $O(n)$, la complessità temporale di $M$ risulta essere $$O(n) \cdot O\rbk{2^{n^k}} = O\rbk{n \cdot 2^{n^k}}$$ e poiché $n^k \ge \log(n)$ (per $k$ sufficientemente grande) si ha che $$O\rbk{n \cdot 2^{n^k}} = O\rbk{2^{n^k}} \implies HAMPATH \in \EXP$$ TODO ASSICURATI CHE SIA TUTTO GIUSTO
    \end{proof}
    ...
    \begin{proof}
        Siano $A$ e $B$ due linguaggi definiti su uno stesso alfabeto, tali che $A \leq_\mathrm L B$, e dunque per definizione esiste una funzione $\func{f}{\Sigma^*}{\Sigma^*}$ calcolabile in spazio logaritmico; allora, per definizione di $f$, esiste un trasduttore di spazio logaritmico $T$ che, su qualsiasi input $w \in \Sigma^*$, se si ferma, termina avendo solo $f(w)$ sul suo \tit{output tape}. Si noti che $T$ computa in spazio logaritmico, e dunque poiché $\LSPACE \subseteq \Pclass$ per il \cref{l in p}, si ha che $T$ è anche una macchina di Turing di tempo polinomiale tale per cui $f$ risulta dunque essere una funzione polinomialmente calcolabile; di conseguenza, $f$ è tale che $A \leq_\mathrm P B$ per definizione di riduzione polinomiale mediante funzione.
    \end{proof}
    ...
    \begin{framedprop}[label={log space ops}, breakable]{Operazioni in spazio logaritmico}
        ...
        \begin{itemize}
            ...
            \item effettuare prodotti tra 2 numeri:
                \begin{itemize}
                    \item la logica è analoga alla somma, pertanto verrà omessa;
                    \item lo spazio richiesto al prodotto è dunque lo spazio richiesto per rappresentare $a \cdot b$, ovvero $O(\log(a \cdot b) = O(\log(a) + \log(b)) = O(\max(\log(a), \log(b)))$
                \end{itemize}
        \end{itemize}
    \end{framedprop}
    ...
    \begin{frameddefn}{Classe \NLSPACE}
        Si definisce \tbf{classe dei linguaggi decidibili non deterministicamente in tempo logaritmico} da una \NTM definita come descritto nell'\cref{space model} il seguente insieme $$\NLSPACE := \NSPACE(\log(n))$$
    \end{frameddefn}
    ...
    \begin{frameddefn}{Complessità di spazio di verificatori}
        Dato un verificatore $V$ definito come descritto nell'\cref{space model}, ed una codifica $\abk{w, c} \in L(V)$, lo spazio di computazione di $V$ è misuato solamente in termini della lunghezza di $w$.
    \end{frameddefn}
    ...
    \begin{proof}
        ...
        \begin{algorithmic}[1]
            \Function{findPathNL}{$G, x, y$}
                ...
                \EndIf
                \State $u := x$
                \For{$v \in V$} \Comment{scelto \underline{non deterministicamente}}
                    \If{$(u, v) \in E$}
                        \State $u := v$
                        \If{$u == y$}
                            \State \textbf{accept}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item sia $R_l$ l'insieme dei vertici raggiungibili da $s$ con al massimo $l$ archi, e sia $r_l := \abs{R_l}$;
            \item $V$ interpreta dunque $c$ come $\abk{c_0, \ldots, c_{\abs{V_G} -1}}$, dove ogni $c_i$ viene interpretato da $V$ come certificato di non appartenenza ad $R_i$ da parte di $t$; in simboli $$\forall i \in [0, \abs{V_G}) \quad c_i = \abk{s \to v_1, \ldots, s \to v_k}$$ per qualche $k\in \N$, dove $v_1, \ldots, v_k \in V_G \mid \forall j \in [1, k] \quad v_j \neq t$ sono tutti i nodi raggiungibili da $s$ attraverso cammini lunghi al massimo $i$; inoltre, i cammini all'interno di ogni $c_i$ devono essere ordinati in ordine di lunghezza (e, per evitare ambiguità, lessicograficamente in base ai vertici $v_j$ a parità di lunghezza); se non è possibile effettuare tale interpretazione per via di una codifica incorretta, $V$ accetta;
            \item dopo aver controllato che tali codifiche sono interpretabili come descritto, $V$ controlla che $c_{i + 1}$ sia un'estensione di $c_i$, per ogni $i \in [0, \abs{V_G})$; se cosi non fosse, $V$ accetta;
            \item $V$ esegue la seguente procedura:
                \begin{algorithmic}[1]
                    \Function{checkPath}{$G, s, t, c$}
                        ...
                        \EndFor
                        \State \textbf{accept}
                        \State TODO METTI LA COSA DEGLI ARCHI
                    \EndFunction
                \end{algorithmic}
        \end{itemize}

        La dimostrazione della correttezza della procedura \ttt{checkPath} di $V$ procede per induzione sui vertici raggiungibili da $s$.
            ...
            \begin{enumerate}[label=]
                ...
                \item \tit{Ipotesi induttiva.} Dato un $l \in [0, \abs{V_G})$, \ttt{checkPath} è in grado di stabilire se $t \notin R_l$. Si noti che, per definizione stessa, si ha che $\forall h \in [0, \abs{V_G}) \quad R_h \subseteq R_{h + 1}$, dunque segue che $$t \notin R_l \implies \forall h \in [0, l] \quad t \notin R_h$$ implicando che i cammini lunghi al più $l$ contenuti in $c_l$ --- che saranno anche i primi $r_l$, poiché si è ipotizzato che la codifica di $c_l$ sia ordinata per lunghezza dei cammini --- non sono cammini $s \to t$.
                \item \tit{Passo induttivo.} È necessario dimostare che per $l +1$, \ttt{checkPath} è in grado di stabilire se $t \notin R_{l + 1}$. Si noti che, per ipotesi induttiva, \ttt{checkPath} è in grado di stabilire se $t \notin R_l$, ed inoltre $R_l \subseteq R_{l + 1}$ per quanto detto in precedenza. Allora, poiché TODO METTI LA COSA DEGLI ARCHI
            \end{enumerate}

            Dunque, questo dimostra che \ttt{checkPath} è in grado di stabilire correttamente se $t \in R_{\abs{V_G}}$; allora, attraverso tale procedura, $V$ stabilisce correttamente se $t$ è raggiungibile da $s$. Di conseguenza, se non esistono cammini $s \to t \in G$, esiste una $c \in \Sigma ^*$ tale che $\abk{\abk{G, s, t}, c} \in L(V)$, e se esiste un cammino $s \to t$ allora \ttt{checkPath} è in grado di stabilire che $t \in R_{\abs{V_G}}$ correttamente, e dunque $V$ rifiuta. Questo dimostra che $V$ è un verificatore per $\overline{PATH}$.

            Inoltre, si noti che TODO COSTO SPAZIALE
    \end{proof}
    ...
    \begin{frameddefn}{Classe \NPSPACE}
        Si definisce \tbf{classe dei linguaggi decidibili non deterministicamente in tempo logaritmico} da una \NTM definita come descritto nell'\cref{space model} il seguente insieme $$\NPSPACE := \bigcup_{k \in \N}{\NSPACE \rbk{n^k}}$$
    \end{frameddefn}
    ...
    \begin{proof}
        Trivialmente si ha che $\PSPACE \subseteq \NPSPACE$, poiché una \TM è una particolare \NTM, inoltre, per il \cref{savitch}, si ha che $$\forall k, h \in \N \mid h = 2k \quad \NSPACE(n^k) \subseteq \DSPACE \rbk{\rbk{n^k}^2} = \DSPACE \rbk{n^{2k}} = \PSPACE(n^h)$$ dunque segue la tesi per doppia implicazione.
    \end{proof}

    \subsection{Classe \NEXPSPACE}

    \begin{frameddefn}{Classe \NEXPSPACE}
        Si definisce \tbf{classe dei linguaggi decidibili non deterministicamente in tempo logaritmico} da una \NTM definita come descritto nell'\cref{space model} il seguente insieme $$\NEXPSPACE := \bigcup_{k \in \N}{\NSPACE \rbk{2^{n^k}}}$$
    \end{frameddefn}
    ...
    \begin{framedobs}[breakable]{Gerarchia delle classi}
        TODO METTI QUESTA OBS NEL POSTO GIUSTO (controlla teoremi di gerarchia) E NOMINA I TEOREMI USATI
