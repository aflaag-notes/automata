...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item poiché $G$ è in CNF in ipotesi, la derivazione $A_j \derives v A_l y$ deve aver necessariamente utilizzato una regola della forma $A_j \to BC$ dove $B \derives v A_l$ e $C \derives y$ oppure $B \derives v$ e $C \derives A_l y$; allora, poiché l'unica $\varepsilon$-regola ammessa in una grammatica in CNF è $S \to \varepsilon$, TODO, segue che $v \neq \varepsilon$ oppure $y \neq \varepsilon$; questo dimostra la seconda condizione della tesi;
            \item poiché $A_j$ si trova tra le ultime $\abs{V} + 1$ variabili del cammino considerato, chiamando $h'$ il suo sottoalbero, esso deve avere altezza al massimo $\abs{V} + 1$; allora, poiché $$h' \le \abs{V} + 1 \iff h' - 1 \le \abs{V}$$ per la \cref{prop pre pump cfl} segue che $\abs{vxy} \le 2^{h' - 1} \le 2^{\abs{V}} =:$ dimostrando dunque la terza condizione della tesi.
        \end{itemize}
    \end{proof}
