    ...
    \begin{proof}[Dimostrazione II]
        ...
        \begin{itemize}
            \item $F$ costruisce una macchina di Turing $M'$ che, data in input una stringa $x$, computa come segue:
                \begin{itemize}
                    \item $F$ esegue $M(x)$
                    \item se $M$ accetta, $F$ accetta;
                    \item se $M$ rifiuta, $F$ cicla (ad esempio muovendo la testina verso una direzione per sempre);
                \end{itemize}
            \item restituisce $\abk{M', w}$ sul suo nastro.
        \end{itemize}

        Si noti dunque che, poiché $F$ si ferma solo se ha $\abk{M', w}$ sul nastro, per qualsiasi input $\abk{M, w}$, la funzione $f$ che $F$ descrive è calcolabile per definizione. Allora, si noti che $$\abk{M, w} \in A_\TM \iff w \in L(M) \implies w \in L(M')$$ per definizione di $A_\TM$ e per costruzione di $M'$ rispettivamente; inoltre $$w \in L(M') \implies f(\abk{M, w}) = \abk{M', w} \in HALT_\TM$$ per definizione di $HALT_\TM$; viceversa, si ha che \centeredeq{0.9}{$\abk{M, w} \notin A_\TM \iff w \notin L(M) \implies M'(w) \ \textrm{va in loop} \implies f(\abk{M, w}) = \abk{M', w} \notin HALT_\TM$} e dunque $$\abk{M, w} \in A_\TM \iff f(\abk{M, w}) \in HALT_\TM$$ Allora, per definizione, segue che $$A_\TM \leq_\mathrm m HALT_\TM$$ e poiché $A_\TM \notin \DEC$ per il \cref{a_tm not in dec}, è verificata la tesi per cui $HALT_\TM \notin \DEC$.
    \end{proof}
