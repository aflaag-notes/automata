...
\begin{document}
    ...
    \begin{frameddefn}[label={ling reg}]{Linguaggio regolare}
        Un linguaggio è detto \tbf{regolare} se e solo se esiste un DFA che lo riconosce. La classe dei linguaggi regolari è denotata con $\mathrm{REG}$.
    \end{frameddefn}
    ...
    \begin{framedprop}{Chiusura dell'elevamento a potenza}
    ...
    \begin{framedprop}{Unione di grammatiche}
    ...
    \begin{example}[Derivazione a sinistra]
        Si consideri la seguente grammatica: $$\centeredsoe{E \to E \ttt + E \\ E \to E \ttt * E \\ E \to \ttt ( E \ttt ) \ | \ \ttt a}$$ Attraverso essa, è possibile ottenere ad esempio la seguente stringa, applicando la derivazione a sinistra: $$E \yields E \ttt * E \yields E \ttt + E \ttt * E \yields \ttt a \ttt + \ttt a \ttt * \ttt a$$
    \end{example}
    ...
    \begin{framedobs}{Linguaggi non regolari e PDA}
        ...
        Dunque, di fatto, il PDA sta \tit{contando} il numero di \ttt 0 e di \ttt 1 presenti all'interno della stringa, poiché la computazione avanza solamente se, per ognuno degli \ttt 1 letti, è possibile rimuovere uno \ttt 0 dallo stack. Allora, l'automa riconosce il linguaggio $$L = \{\ttt 0 ^n \ttt 1 ^n \mid n \in \N\}$$ il quale non è regolare per l'\cref{non reg ex}.
    \end{framedobs}
    ...
    \begin{example}[PDA]
        ...
        Esso è in grado di riconoscere il linguaggio $$L=\{w \in \{\ttt 0, \ttt 1\}^* \mid w \ttt \& w ^\mathcal{R}\}$$ poiché TODO
    \end{example}

    \begin{framedmeth}[breakable]{Trasformare CFG in PDA}
        \label{cfg into pda}
    ...
    \subsection{Configurazioni di PDA}
