...
\begin{document}
    ...
    \begin{frameddefn}{Linguaggio di accettazione}
        Si definiscono \tbf{linguaggi di accettazione} i linguaggi definiti come segue $$A_\mathcal{C} := \{\abk{B, w} \mid B \in \mathcal{C} , w \in L(B)\}$$ Dunque, per sua definizione, controllare che l'oggetto $B \in \mathcal{C}$ accetti $w$ equivale a controllare che $\abk{B,w}$ sia in $A_\mathcal{C}$.
    \end{frameddefn}
    ...
    \begin{proof}
        Se una grammatica $G$ è in CNF, allora ogni stringa $w$, avente lunghezza $|w| := n \ge 1$, derivabile attraverso le regole di $G$, richiede esattamente $2n - 1$ passaggi. La dimostrazione procede per induzione sulla lunghezza della stirnga $w$.
        ...
        \begin{itemize}
            ...
            \item se la stringa $w$ ha lunghezza $n \ge 1$, la macchina di Turing che esegue tale algoritmo lista tutte le derivazioni di $G$ in CNF aventi lunghezza $2n - 1$, e se $w$ è presente in una di queste la \TM accetta, altrimenti rifiuta.
        \end{itemize}
    ...
    \begin{frameddefn}{Linguaggio del vuoto}
        Si definiscono \tbf{linguaggi del vuoto} i linguaggi definiti come segue $$E_\mathcal{C} := \{\abk{A} \mid A \in \mathcal{C}: L(A) = \emptyset\}$$ Dunque, per sua definizione, controllare che il linguaggio di $A \in \mathcal{C}$ sia vuoto equivale a controllare che $\abk{A}$ sia in $E_\mathcal{C}$.
    \end{frameddefn}

    \begin{framedthm}{Decidibilità di $E_\DFA$}
        $E_\DFA$ è decidibile; in simboli $$E_\DFA := \{\abk{A} \mid A \in \DFA: L(A) = \emptyset \}$$
    \end{framedthm}
